...
import { queryTogetherAI } from '../api/together';
...
const Chat = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const botResponse = await queryTogetherAI(userMessage);
      setMessages((prev) => [...prev, { role: 'bot', text: botResponse }]);
    } catch (error) {
      setMessages((prev) => [...prev, { role: 'bot', text: '❌ Failed to get a response. You may have hit the free limit or there’s a temporary issue. Please try again later.' }]);
    }
      ...
      <div className="flex flex-col min-h-screen bg-white dark:bg-[#1e1e1e] text-black dark:text-white">
        ...
        <footer className="p-4 border-t border-gray-200 dark:border-gray-700 w-full">
          <div className="mx-auto flex w-full max-w-2xl items-center gap-2 sm:flex-nowrap flex-wrap">
            ...
            <button
            onClick={handleSend}
            className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl transition flex items-center justify-center h-10"
            title="Send"
            disabled={loading}
          >
          ...
          <button
            ...
            className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-xl flex items-center justify-center transition h-10"
            title="Voice input"
          >
